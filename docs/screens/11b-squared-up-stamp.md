...
Figma node `172:3255`. Built in `src/screens/SquaredUpStamp.tsx`. **Do not re-fetch.**
...
`CLAUDE.md`'s "How to work" §4), so this is the "B" state as its own reachable screen for now
(`/?screen=squared-up-stamp`).

## Reused the existing `Stamp` component, not Figma's raw stamp tree
`get_design_context` on this node returns the entire Stamp component's internals exploded into ~40
...
its flat PNG (`stamps.portugal`, per `docs/DESIGN_SYSTEM.md` §7) with the correct crop and
...
Left the drop-from-scale-1.3 entrance motion (`docs/DESIGN_SYSTEM.md` §6) for the later motion pass
...
- Nearly all of this node's `get_design_context` output was the Stamp component's own internals —
